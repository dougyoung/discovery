...
lazy val datasetUrl = new URL("http://alaska.epfl.ch/~dockermoocs/bigdata/stackoverflow.csv")
lazy val datasetPath = Paths.get("src/main/resources/stackoverflow/stackoverflow.csv")

def downloadIfNotExists(url: URL, path: Path): Unit = {
  if (Files.notExists(datasetPath)) {
    println(s"Downloading $url to $path")
    datasetPath.getParent.toFile.mkdirs
